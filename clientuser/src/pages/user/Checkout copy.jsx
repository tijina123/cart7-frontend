...
const Checkout = () => {
  const { getCartPageData, postOrder, postAddress, checkPorductAvailability } = UserService()
  ...
  const getData = async (userId = '67bffce8e7dae9e3c14f3c15') => {
    try {
      const response = await getCartPageData(userId);
      console.log(response, "-----fechcart");
  ...
  // const useCheckCartAvailability = () => {
  //   const [cartStatus, setCartStatus] = useState(null);
  //   // const [loading, setLoading] = useState(false);

  //   return { cartStatus, loading, error, checkCartAvailability };
  // };

  const checkCartAvailability = async () => {

    let userId = "67bffce8e7dae9e3c14f3c15"

    try {
      // const response = await axios.get(`http://localhost:3000/order/check-cart/${userId}`);
      // const response = await checkPorductAvailability(address);
      console.log(response, "=====response");


      // if (response.status === 200) {
      //   setCartStatus({ available: true, message: response.data.message });
      // }
    } catch (err) {
      if (err.response && err.response.status === 400) {
        setCartStatus({
          ...
          unavailableProducts: err.response.data.unavailableProducts,
        });
      } else {
        // setError("Failed to check cart availability. Please try again.");
      }
    } finally {
      // setLoading(false);
    }
  ...
  const handleSubmitAddress = async () => {
    ...
    try {
      ...
      //   ...address,
      //   user: '67bffce8e7dae9e3c14f3c15', // Pass the user ID from props
      // });
  ...
  //     const orderData = {
  //       "_id": "65f0a1c9a9b6c7001cf8e7d2",
  //       "user": "65f09f74a9b6c7001cf8e7d1",
  //       "orderItems": [
  //         {
  //           "product": "65f0a1c9a9b6c7001cf8e7a1",
  //           "quantity": 2
  //         },
  //         {
  //           "product": "65f0a1c9a9b6c7001cf8e7a2",
  //           "quantity": 1
  //         }
  //       ],
  //       "shippingAddress": {
  ...
  //       "deliveredAt": "2025-03-05T14:30:00.000Z",
  //       "shipment_id": "SR123456",
  //       "awb_code": "AWB987654",
  //       "courier_name": "FedEx",
  //       "courier_status": "In Transit",
  //       "createdAt": "2025-03-04T12:00:00.000Z",
  //       "updatedAt": "2025-03-05T10:15:00.000Z"
  //     };
  //     setOrder(orderData);
  //   };
